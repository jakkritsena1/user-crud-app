...
import { getData, pushData, postData, deleteData } from '../hooks/admin-api.js'
...
export default function Table() {
    ...
    const Push = async (e) => {
        e.preventDefault();
        if (newRow.name.trim() && newRow.lastname.trim() && newRow.user.trim() && newRow.password.trim()) {
            try {
                setNewRow({ name: '', lastname: '', user: '', password: '' });
                const res = await postData(newRow.name, newRow.lastname, newRow.user, newRow.password);
        ...
        const Update = async (e, index) => {
            ...
            try {
                ...
                const res = await pushData(row.id, row.name, row.lastname, row.user, row.password);
                        ...
                        <tbody>
                            {rows.map((item, index) => (
                                <tr className='data' key={item.id}>
                                    ...
                                    <td><input type='text' placeholder='user' name='user' value={item.user} onChange={e => changeData(index, 'user', e.target.value)} required /></td>
                                    <td><input type='text' placeholder='password' name='password' value={item.password} onChange={e => changeData(index, 'password', e.target.value)} required /></td>
